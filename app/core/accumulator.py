...
class AccumulatorClass:
    def __init__(self):
        ...
        envModulus = settings_dependency().ACC_MODULUS
        envGenerator = settings_dependency().ACC_GENERATOR

        if envModulus > 0 and envGenerator > 0:
    ...
    def setup(self, modulus: int = None, generator: int = None):
        ...
        if not generator or generator == 0:
    ...
    def add(self, x):
        ...
        if x in self.SECRETS_DICT.keys():
            # data = {"accVaL": to_padded_num_str(self.prevAccumulator, 384), "proof": None, "prime": None}
            data = {
                "dataHash": x,
                "accVal": to_padded_num_str(self.currentAccumulator, 384),
                "proof": None,
                "prime": None,
            }
        else:
            ...
            if debug >= 0:
                ...
                print(f"\n\n\t\tPaddedStrProof: {paddedStrProof}")
                print(f"\n\t\tPaddedStrPrime: {paddedStrPrime}")
                print(f"\n\t\tPaddedStrAccumulator: {paddedStrAccumulator}")

            data = {
                "dataHash": x,
                "accVal": paddedStrAccumulator,
                "proof": paddedStrProof,
                "prime": paddedStrPrime,
            }
    ...
    def verify_exponentiation(self, Q, l_nonce, u, x, x_nonce, w):
        """
        ...
        return self.__verify_exponentiation(Q, l_nonce, u, x, w, self.MODULUS)
    ...
    def verify_membership(self, accVal, proof, prime):
        """
        ...
        try:
            ...
            proof = int(proof, 16)
            # x = hash_to_prime(
            #     x=x, num_of_bits=self.ACCUMULATED_PRIME_SIZE, nonce=nonce
            # )[0]
            accVal = int(accVal, 16)
            prime = int(prime, 16)

            print(f"[verify_membership] Before __verify_membership: \n")
            print(f"Accumulator: {accVal}")
            print(f"prime: {prime}")
            print(f"proof: {proof}")


            result = self.__verify_membership(
                accVal,
                prime,
                proof,
                self.MODULUS,
            )
            print(f"[verify_membership] result: {result}")
        except Exception as e:
            print(f"[ERR] Error in verify_membership: {e}")
            return False
        return result

    # def verify_membership(self, x, proof):
    #     """
    #     Fail: only works for the latest accVal
    #     Verify membership proof
    #     :param x: hash of element to verify membership for
    ...
    #         )[0]
    #         print(
    #             f"[verify_membership] Before __verify_membership: \n"
    #             f"Accumulator: {self.currentAccumulator}\n"
    #             f"x: {x}\n"
    #             f"proof: {proof}\n"
    ...
    # def verify_membership(self, x, proof):
    #     """
    #     NOTE: This is the og func, relies on the original data to be passed in
    #     Verify membership proof
    ...
    def __verify_membership(self, A, x, proof, n):
        ...
        :return: True if proof is valid, False otherwise
        """
        print(f"pow(proof, x, n): {pow(proof, x, n)}")
        print(f"A: {A}")
...
#     def save_acc_to_file(self, filename):
#         """Save the Accumulator to a file."""
#         print(f"[CORE] Accumulator Print.")
#         print(f"[CORE] Accumulator: {self.currentAccumulator}")
...
#                             new_instance = AccumulatorClass()
#                             new_instance.accumulator = (
#                                 accumulator  # Assign the loaded tree
#                             )
#                             return new_instance
#                         elif isinstance(
#                             accumulator, AccumulatorClass
...
# accumulator_file = "accumulator.pkl"
# if os.path.exists(accumulator_file):
#     merkleCore = AccumulatorClass.load_tree_from_file(accumulator_file)
# else:
#     merkleCore = AccumulatorClass()
...
dataHash, newAcc, proof, prime = accumulatorCore.add("1")
# print(f"\n\tnewAcc: {newAcc}\n\tproof: {proof}\n\tprime: {prime}")
