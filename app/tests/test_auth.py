...
async def test_auth_register_endpoint():
    """Test auth register endpoint exists and handles requests."""
    async with AsyncClient(
        transport=ASGITransport(app=app), base_url="http://localhost:8000"
    ) as ac:
        response = await ac.post(
            "/auth/v1/register", json={"email": "test@example.com", "password": "test"}
        )
        # Should return some response (not 404), meaning endpoint exists
        assert response.status_code != 404
